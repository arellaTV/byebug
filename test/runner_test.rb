...
module Byebug
  ...
  class RunnerAgainstValidProgramTest < TestCase
    ...
    def test_run_with_a_script_and_params_does_not_consume_script_params
      stdout = run_program(
        "bin/byebug -- #{example_path} -opt value",
        'puts "Args: #{ARGV.join(\', \')}"'
      )
    ...
    def test_run_with_post_mortem_mode_flag
      stdout = run_program("bin/byebug -m #{example_path}", 'show post_mortem')

      assert_match(/post_mortem is on/, stdout)
    end

    def test_run_with_linetracing_flag
      stdout = run_program("bin/byebug -t #{example_path}", 'show linetrace')
    ...
    def test_run_with_no_quit_flag
      skip
      stdout = run_program("bin/byebug --no-quit #{example_path}", 'quit!')
    ...
    def test_run_with_require_flag
      stdout = run_program(
        "bin/byebug -r abbrev #{example_path}",
    ...
    def test_run_with_a_single_include_flag
      stdout = run_program(
        "bin/byebug -I dir1 #{example_path}",
    ...
    def test_run_with_several_include_flags
      stdout = run_program(
        "bin/byebug -I d1:d2 #{example_path}",
    ...
    def test_run_with_no_stop_flag_does_not_stop_at_the_first_line
      stdout = run_program("bin/byebug --no-stop #{example_path}")

      refute_match(/=> 1: sleep 0/, stdout)
    end

    def test_run_with_stop_flag_stops_at_the_first_line
      stdout = run_program("bin/byebug --stop #{example_path}")
